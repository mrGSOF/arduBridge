...
class test():
    ...
    def printHelp(self):
        print("Use the below:")
        print("test.config()      #< Initiate the display")
        print("test.cls()         #< Clease the display")
        print("test.image(v=True) #< Display an image")
        print("test.animateText('HELLO WORLD', v=True)")
    ...
    def image(self, filename='happycat_oled_64.ppm', scale=True, v=False):
        ...
        self.disp.display()
        if v:
            print("Transfer took %1.1f ms"%(1000*(time.time() -T0)))
        
    def animateText(self, text=False, dt=0, v=False):
        if test == False:
        ...
        while True:
            ...
            if pos < -maxwidth:
                pos = startpos
            if dt > 0:
                time.sleep(dt) #< Pause briefly before drawing next frame.
