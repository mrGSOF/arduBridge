...
def testDirect(pwmCh, N=2, acc=800, dt=0.05):
    ards.servoMode(pwmCh, 1)
    for i in range(0,N):
        ards.servoScurveDirect(pwmCh, p0=95, p1=250, acc=acc, dt=dt)
        ards.servoScurveDirect(pwmCh, p0=250, p1=95, acc=acc, dt=dt)
        ards.servoScurveDirect(pwmCh, p0=95, p1=0, acc=acc, dt=dt)
        ards.servoScurveDirect(pwmCh, p0=0, p1=95, acc=acc, dt=dt)
...
if __name__ == "__main__":
    ...
    pwmCh = 0
    ards.servoMode(pwmCh, ardu.gpio.SERVO) #< Servo on PWM connector#2
    #ards.servoMode(3,ardu.gpio.SERVO) #< Servo on PWM connector#3

    ### Set the servo position 
    ards.servoSet(pwmCh,50)               #< Step transition to target position (0 to 250)
    #ards.servoSet(3,170)
    time.sleep(1.0)
    ards.servoScurve(pwmCh, 50, 170, 800) #< Smooth transition from -> to positions 
    ...
    print("ards.servoScurveDirect(pwmCh, p0=0, p1=250, acc=800, dt=0.05)")
    print("ardu.gpio(pin, p0=0, p1=250, acc=800, dt=0.05)")
    print("test(pwmCh, N=2, acc=800, dt=0.05)")
    print("testDirect(pwmCh, N=2, acc=200, dt=0.01)")
