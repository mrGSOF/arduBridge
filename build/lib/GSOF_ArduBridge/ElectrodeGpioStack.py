...
class ExtGpioStack():
    def __init__(self, i2c=False, devList=[], logger=None, pin2pin=False):
    ...
    def init(self):
        ...
        for dev in self.devList:
            ...
            if self.i2c:
                self.ExtGpio.append( Max3700ExtGPIO.Max3700ExtGPIO( i2c=self.i2c, devID=dev, logger=self.logger ) )
    ...
    def pinMode(self, pin, mode):
        ...
        if self.logger != None:
            self.logger.debug(f"ExtDir{pin}: {self.DIR[mode]} - {self.RES[reply]}")
        return reply[0]
    ...
    def pinWrite(self, pin, valList):
        ...
        for val in valList:
            ...
            if dev != -1:
                ...
                if self.logger != None:
                    self.logger.debug(f"ExtPinSet{pin}: {val} - {self.RES[reply]}")
            else:
                if self.logger != None:
                    self.logger.error(f"ExtPinSet{pin}: Out of range")
                return -1
            pin += 1
        return 1
    ...
    def pinRead(self, pinList) -> list:
        ...
        for pin in pinList:
            ...
            if dev != -1:
                ...
                val = reply[0]
                if self.logger != None:
                    self.logger.debug(f"ExtPin{pin}: {val}")
                result.append(val)
            else:
                if self.logger != None:
                    self.logger.error(f"ExtPin{pin}: Out of range")
                return -1
        return result
